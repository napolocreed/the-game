...
const cellClass = (status: CompletionStatus | null, scheduled: boolean): { cls: string; h: string } => {
  if (status === CompletionStatus.COMPLETED) return { cls: 'bg-good-soft', h: '100%' };
  if (status === CompletionStatus.FAILED) return { cls: 'bg-cat-health', h: '45%' };
  if (status === CompletionStatus.SKIPPED) return { cls: 'bg-neutral', h: '70%' };
  if (scheduled) return { cls: 'bg-inset-deep', h: '100%' }; // scheduled, no record → visible gap
  ...
  return (
    <div className="bg-surface border-4 border-frame p-3 shadow-hard">
      <div className="flex items-center gap-2 mb-2">
        ...
        />
        <span className="text-sm text-ink-hi truncate flex-1 min-w-0">{habit.name}</span>
        <span className="text-lg text-accent shrink-0">{pct !== null ? `${pct}%` : '—'}</span>
      </div>

      <div className="h-6 flex items-end gap-[2px] bg-inset border-2 border-shadowc p-[2px]">
        {cells.map(cell => {
